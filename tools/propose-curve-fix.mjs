...
const MAX_HP = MED_HP * 2.0;
const MIN_HP = MED_HP / 2.0;
const MAX_ATK = MED_ATK * 1.8;

console.log(
  `mediane boss/std : pv x${MED_HP.toFixed(1)}  atk x${MED_ATK.toFixed(2)}`,
);
console.log(
  `bande retenue    : pv [x${MIN_HP.toFixed(1)} , x${MAX_HP.toFixed(1)}]  atk max x${MAX_ATK.toFixed(2)}\n`,
);
...
rows.forEach((r, i) => {
  ...
  let hp = null;
  if (r.bHp / stdFinal > MAX_HP)
    hp = Math.round((stdFinal * MAX_HP) / 100) * 100;
...
for (const f of fixes) {
  console.log(
    (MONSTERS[f.bossId].name || f.bossId).slice(0, 33).padEnd(34) +
      String(f.bHp).padStart(9) +
      " ->" +
      String(f.hp ?? f.bHp).padStart(9) +
      (
        "  x" +
        f.ratio.toFixed(1) +
        " -> x" +
        ((f.hp ?? f.bHp) / f.stdFinal).toFixed(1)
      ).padEnd(20) +
      (f.stdFinal !== f.sHp
        ? ` [std ${Math.round(f.sHp)}->${f.stdFinal}]`
        : "") +
      (f.atk ? `  ${f.bAtk} -> ${f.atk}` : ""),
  );
}
...
console.log(
  JSON.stringify(
    fixes.map((f) => ({ id: f.bossId, hp: f.hp, atk: f.atk })),
    null,
    0,
  ),
);
