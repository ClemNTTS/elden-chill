export const LOOT_TABLES = {
  /*===========================
    Tier 0 -> 1 Biomes
  ============================*/
  limgrave_west: [
    { id: "iron_sword", chance: 0.2 },
    { id: "crimson_amber", chance: 0.3 },
    { id: "scholars_ring", chance: 0.2 },
    { id: "kama", chance: 0.3 },
  ],

  /*===========================
    Tier 1 -> 2 Biomes
  ============================*/
  limgrave_east: [
    { ashId: "bloody_slash", chance: 0.2 },   // Entaille Sanglante : ~6 runs
    { id: "bloodhound_fang", chance: 0.4 },
    { id: "leather_vest", chance: 0.2 },
    { id: "kama", chance: 0.4 },
  ],
  limgrave_north: [
    { id: "margit_shackle", chance: 0.2 },
    { id: "knight_greatsword", chance: 0.2 },
    { id: "briar_armor", chance: 0.4 },
    { id: "margit_hammer", chance: 0.02 },
  ],
  limgrave_lake: [
    { id: "burned_dragon_hearth", chance: 0.15 },
    { id: "burn_sword", chance: 0.5 },
  ],
  caelid_west: [
    { ashId: "great_shield", chance: 0.1 },
    { id: "keen_dagger", chance: 0.6 },
    { id: "leather_boots", chance: 0.3 },
  ],
  /*===========================
    Tier 3
  ============================*/
  weeping_peninsula: [
    { ashId: "great_shield", chance: 0.2 },   // Rempart Inebranlable : ~6 runs
    { id: "zamor_curved_sword", chance: 0.5 },
    { id: "radagon_scarseal", chance: 0.5 },
  ],

  enter_stormwind_castle: [
    { ashId: "storm_stomp", chance: 0.2 },   // Pietinement Tempetueux : ~6 runs
    { id: "twin_blade", chance: 0.5 },
    { id: "forged_grip", chance: 0.5 },
  ],

  stormwind_castle: [
    { id: "godrick_knight_armor", chance: 0.1 },
    { id: "godrick_great_rune", chance: 0.4 },
    { id: "godrick_axe", chance: 0.5 },
  ],

  morne_castle: [
    { id: "rune_fragment", chance: 0.2 },
    { id: "pumkin_helm", chance: 0.2 },
    { id: "grafted_blade_greatsword", chance: 0.6 },
  ],

  caelid_west: [
    { id: "rotten_greataxe", chance: 0.15 },
    { id: "vermilion_seed", chance: 0.2 },
    { id: "sage_caelid_robe", chance: 0.3 },
    { id: "winged_sword_insignia", chance: 0.35 },
  ],
  liurnia_south: [
    { id: "carian_glintstone_staff", chance: 0.2 },
    { id: "moon_of_nokstella", chance: 0.2 },
    { id: "icerind_hatchet", chance: 0.3 },
    { id: "black_knife_gauntlets", chance: 0.3 },
  ],
  raya_lucaria_academy: [
    { id: "academy_glintstone_staff", chance: 0.3 },
    { id: "carian_crusher", chance: 0.3 },
    { id: "karolos_mask", chance: 0.4 },
  ],
  liurnia_west: [
    { id: "raya_lucaria_robe", chance: 0.4 },
    { id: "heavy_crystal_gauntlets", chance: 0.3 },
    { id: "crystal_crust_armor", chance: 0.3 },
  ],

  liurnia_east: [
    { id: "crystal_crust_armor", chance: 0.5 },
    { id: "carian_crusher", chance: 0.5 },
    { id: "bog_amulet", chance: 0.4 },
  ],

  liurnia_marsh: [{ id: "glintstone_dragon_heart", chance: 1 }],

  caelid_south: [
    { id: "executioner_greataxe", chance: 0.2 },
    { id: "executioner_hood", chance: 0.2 },
    { id: "guillotine_pendant", chance: 0.2 },
    { id: "marionette_scimitar", chance: 0.4 },
  ],

  redmane_castle: [
    { id: "radahn_lion_armor", chance: 0.5 },
    { id: "starscourge_greatsword", chance: 0.5 },
  ],
  caelid_dragonbarrow: [{ id: "rotten_dragon_heart", chance: 1 }],

  altus_plateau: [
    { ashId: "executioners_step", chance: 0.16 },
    { id: "golden_tree_halberd", chance: 0.3 },
    { id: "golden_sentinel_armor", chance: 0.3 },
    { id: "sentinel_greatshield_talisman", chance: 0.4 },
    { id: "altus_exec_blade", chance: 0.18 },
    { id: "altus_exec_cloak", chance: 0.22 },
    { id: "altus_exec_sigil", chance: 0.14 },
  ],

  mount_gelmir: [
    { id: "gelmir_dragon_fang", chance: 0.22 },
    { id: "gelmir_dragon_hide", chance: 0.26 },
    { id: "gelmir_dragon_eye", chance: 0.12 },
    { id: "talisman_storm_dragon", chance: 0.2 },
    { id: "rune_fragment", chance: 0.2 },
  ],

  mountaintops: [
    { id: "giant_breaker_maul", chance: 0.2 },
    { id: "arena_colossus_plate", chance: 0.24 },
    { id: "arena_colossus_token", chance: 0.18 },
    { id: "talisman_posture", chance: 0.18 },
    { id: "rune_fragment", chance: 0.2 },
  ],

  leyndell_royal: [
    { id: "altus_exec_sigil", chance: 0.2 },
    { id: "talisman_posture", chance: 0.2 },
    { id: "arena_colossus_plate", chance: 0.18 },
    { id: "rune_fragment", chance: 0.2 },
  ],

  forbidden_land: [
    { id: "talisman_wayfarer", chance: 0.22 },
    { id: "arena_colossus_token", chance: 0.18 },
    { id: "giant_breaker_maul", chance: 0.16 },
    { id: "rune_fragment", chance: 0.2 },
  ],

  consecrated_snowfield: [
    { id: "arena_colossus_plate", chance: 0.2 },
    { id: "talisman_posture", chance: 0.2 },
    { id: "talisman_blackrot", chance: 0.16 },
    { id: "rune_fragment", chance: 0.2 },
  ],

  mohgwyn_palace: [
    { id: "azula_black_censer", chance: 0.2 },
    { id: "talisman_blackrot", chance: 0.2 },
    { id: "azula_black_veil", chance: 0.18 },
    { id: "rune_fragment", chance: 0.2 },
  ],

  miquella_haligtree: [
    { id: "azula_black_idol", chance: 0.2 },
    { id: "azula_black_veil", chance: 0.2 },
    { id: "talisman_wayfarer", chance: 0.18 },
    { id: "talisman_blackrot", chance: 0.18 },
  ],

  crumbling_farum_azula: [
    { id: "azula_black_censer", chance: 0.18 },
    { id: "azula_black_veil", chance: 0.24 },
    { id: "azula_black_idol", chance: 0.16 },
    { id: "talisman_blackrot", chance: 0.18 },
    { id: "talisman_wayfarer", chance: 0.24 },
  ],

  caria_mansion: [
    { id: "loretta_glintstone_sickle", chance: 0.3 },
    { id: "lunar_resilience_talisman", chance: 0.7 },
  ],

  siofra_river: [
    { id: "ancestral_renaissance_horn", chance: 0.05 },
    { id: "ancient_bone_axe", chance: 0.45 },
    { id: "ancestral_spirit_horn", chance: 0.5 },
  ],

  nokron: [
    { ashId: "astral_shatter", chance: 0.16 },
    { id: "celestial_dew_talisman", chance: 0.5 },
    { id: "nokron_flame_dagger", chance: 0.5 },
  ],
  ainsel_river: [
    { id: "ainsel_shard_spear", chance: 0.34 },
    { id: "ainsel_starmap", chance: 0.33 },
    { id: "ainsel_silk_robe", chance: 0.33 },
  ],
  deeproot_depths: [
    { ashId: "rootward_vow", chance: 0.16 },
    { id: "rootbound_maul", chance: 0.34 },
    { id: "rootbound_plate", chance: 0.33 },
    { id: "prince_bark_talisman", chance: 0.33 },
  ],
  rotlake: [
    { id: "rotbloom_blade", chance: 0.34 },
    { id: "rotbloom_mail", chance: 0.33 },
    { id: "rotbloom_idol", chance: 0.33 },
  ],
  dominula_village: [
    { ashId: "madding_toll", chance: 0.18 },
    { id: "celebrant_sickle", chance: 0.24 },
    { id: "festival_garb", chance: 0.26 },
    { id: "madding_charm", chance: 0.16 },
    { id: "rune_fragment", chance: 0.2 },
  ],
  shaded_castle: [
    { ashId: "briar_riposte", chance: 0.18 },
    { id: "briar_greatsword", chance: 0.22 },
    { id: "shaded_pauldron", chance: 0.26 },
    { id: "briar_thorn_seal", chance: 0.16 },
    { id: "rune_fragment", chance: 0.2 },
  ],
  volcano_manor: [
    { ashId: "magma_eruption", chance: 0.18 },
    { id: "magma_whip", chance: 0.22 },
    { id: "serpent_scale_mail", chance: 0.26 },
    { id: "manor_contract_seal", chance: 0.16 },
    { id: "rune_fragment", chance: 0.2 },
  ],
  rykard_lair: [
    { id: "godslayer_greatsword", chance: 0.2 },
    { id: "serpent_king_crown", chance: 0.24 },
    { id: "blasphemous_chalice", chance: 0.14 },
    { id: "rune_fragment", chance: 0.2 },
  ],
  divine_tower: [
    { id: "gargoyle_halberd", chance: 0.22 },
    { id: "sealed_plate", chance: 0.26 },
    { id: "tower_seal_ring", chance: 0.16 },
    { id: "rune_fragment", chance: 0.2 },
  ],
  castle_sol: [
    { ashId: "sleep_pot", chance: 0.18 },
    { id: "eochaid_dancing_blade", chance: 0.2 },
    { id: "night_cloak", chance: 0.24 },
    { id: "starlight_shard", chance: 0.16 },
    { id: "rune_fragment", chance: 0.2 },
  ],
  giants_catacombs: [
    { id: "zamor_ice_curved", chance: 0.22 },
    { id: "giant_crusher_plate", chance: 0.26 },
    { id: "zamor_ice_seal", chance: 0.16 },
    { id: "rune_fragment", chance: 0.2 },
  ],
  elphael: [
    { id: "cleanrot_spear", chance: 0.2 },
    { id: "haligtree_crest_shield", chance: 0.22 },
    { id: "scarlet_bloom_charm", chance: 0.16 },
    { id: "rune_fragment", chance: 0.2 },
  ],
  farum_azula_deep: [
    { ashId: "destined_cut", chance: 0.18 },
    { id: "blade_of_destined_death", chance: 0.18 },
    { id: "black_beast_mantle", chance: 0.22 },
    { id: "destined_death_rune", chance: 0.15 },
    { id: "rune_fragment", chance: 0.2 },
  ],
  leyndell_ash: [
    { id: "ozz_grimoire", chance: 0.18 },
    { id: "all_knowing_helm", chance: 0.2 },
    { id: "ashen_capital_seal", chance: 0.15 },
    { id: "rune_fragment", chance: 0.2 },
  ],
  erdtree_throne: [
    { ashId: "golden_vow", chance: 0.18 },
    { id: "radagon_hammer", chance: 0.16 },
    { id: "elden_remembrance_plate", chance: 0.18 },
    { id: "golden_order_seal", chance: 0.14 },
    { id: "rune_fragment", chance: 0.2 },
  ],
  bestial_sanctum: [
    { id: "bestial_sanction", chance: 0.3 },
    { id: "beast_hide_cloak", chance: 0.32 },
    { id: "deathroot_charm", chance: 0.24 },
    { id: "rune_fragment", chance: 0.24 },
  ],
  jarburg: [
    { id: "warrior_jar_shard", chance: 0.34 },
    { id: "jar_lid_shield", chance: 0.36 },
    { id: "jar_luck_charm", chance: 0.26 },
    { id: "rune_fragment", chance: 0.3 },
  ],
  evergaol_champions: [
    { id: "dragon_halberd_ancient", chance: 0.2 },
    { id: "ancient_dragon_scale", chance: 0.22 },
    { id: "lansseax_glaive_ring", chance: 0.16 },
    { id: "rune_fragment", chance: 0.2 },
  ],
};

export const BIOMES = {
  limgrave_west: {
    name: "Nécrolimbe Ouest",
    rareMonsters: ["beastman1"],
    maxRareSpawns: 2,
    monsters: ["soldier1", "wolf1"],
    boss: "troll1_boss",
    length: 10,
    unlocks: ["limgrave_north", "limgrave_east", "limgrave_lake"],
  },
  limgrave_east: {
    name: "Nécrolimbe Est",
    rareMonsters: ["runeBear1", "troll1_duo"],
    maxRareSpawns: 1,
    monsters: ["kaiden_sellsword", "godrick_knight1"],
    boss: "bloodhound_knight_darriwil",
    length: 10,
    unlocks: ["weeping_peninsula", "caelid_west"],
  },
  limgrave_north: {
    name: "Valorage",
    rareMonsters: ["bell_bearing_hunter1", "crucible_knight1"],
    maxRareSpawns: 1,
    monsters: ["white_wolf", "kaiden_sellsword", "godrick_knight1"],
    boss: "margit",
    length: 10,
    unlocks: ["enter_stormwind_castle"],
    //unlocks: ["enter_stormwind_castle", "liurnia_south"],
  },
  limgrave_lake: {
    name: "Lac de Nécrolimbe",
    rareMonsters: ["noble_mage"],
    maxRareSpawns: 6,
    monsters: ["noble_sword", "giant_crab"],
    boss: "limgrave_dragon",
    length: 6,
    unlocks: null,
  },

  /*===========================
    Tier 3
  ============================*/
  weeping_peninsula: {
    name: "Péninsule larmoyante",
    rareMonsters: ["nighth_cavalery", "half_human_queen"],
    maxRareSpawns: 3,
    monsters: ["servant_poison", "bats"],
    boss: "hero_of_zamor",
    length: 11,
    unlocks: ["morne_castle"],
  },
  morne_castle: {
    name: "Château de Vent-Hurlant",
    rareMonsters: ["lesser_mad_pumkin_head"],
    maxRareSpawns: 2,
    monsters: ["misbegotten_servant", "misbegotten_warrior"],
    boss: "misbegotten_leonine",
    length: 8,
    unlocks: null,
  },
  enter_stormwind_castle: {
    name: "Entrée de Voile Orage",
    rareMonsters: ["banished_knight"],
    maxRareSpawns: 2,
    monsters: ["exile_soldier1", "exile_soldier2", "exile_soldier3"],
    boss: "grafted_scion",
    length: 10,
    unlocks: ["stormwind_castle"],
  },
  stormwind_castle: {
    name: "Château de Voile Orage",
    rareMonsters: ["banished_knight"],
    maxRareSpawns: 6,
    monsters: ["stormveil_hawk"],
    boss: "godrick",
    length: 6,
    unlocks: ["liurnia_south"],
  },

  caelid_west: {
    name: "Entrée de Caélid",
    rareMonsters: ["caelid_knight", "crystal_snail"],
    maxRareSpawns: 5,
    monsters: ["rotten_stray", "kindred_of_rot"],
    boss: "commander_oneil_weak",
    length: 14,
    unlocks: null,
    unlocks: ["caelid_south", "caelid_dragonbarrow"],
  },

  /*===========================
    Tier 4
  ============================*/
  liurnia_south: {
    name: "Liurnia des Lacs (Sud)",
    rareMonsters: ["giant_lobster"],
    maxRareSpawns: 3,
    monsters: ["clayman", "raya_sorcerer"],
    boss: "red_wolf_radagon",
    length: 10,
    unlocks: ["liurnia_east", "liurnia_west", "liurnia_marsh"],
  },

  liurnia_marsh: {
    name: "Marais de Liurnia",
    rareMonsters: ["giant_lobster"],
    maxRareSpawns: 5,
    monsters: ["clayman"],
    boss: "liurnia_dragon_smarag",
    length: 6,
    unlocks: null,
  },

  liurnia_east: {
    name: "Liurnia des Lacs (Est)",
    monsters: ["raya_sorcerer", "marionette_soldier"],
    rareMonsters: ["abductor_virgin", "fingercreeper_large"],
    maxRareSpawns: 3,
    boss: "bell_bearing_hunter_liurnia",
    length: 10,
    unlocks: ["raya_lucaria_academy"],
  },

  liurnia_west: {
    name: "Liurnia des Lacs (Ouest)",
    monsters: ["marionette_soldier", "raya_sorcerer"],
    rareMonsters: ["abductor_virgin", "fingercreeper_large"],
    maxRareSpawns: 3,
    boss: "carian_knight_bols",
    length: 10,
    unlocks: ["raya_lucaria_academy"],
  },

  raya_lucaria_academy: {
    name: "Académie de Raya Lucaria",
    rareMonsters: ["living_jar_large"],
    maxRareSpawns: 2,
    monsters: ["marionette_soldier", "raya_sorcerer"],
    boss: "rennala",
    length: 12,
    unlocks: ["altus_plateau", "caria_mansion"],
  },

  altus_plateau: {
    name: "Plateau d'Altus",
    monsters: ["leyndell_soldier", "altus_omen", "altus_praetor_guard"],
    rareMonsters: ["tree_sentinel_altus", "wormface_altus", "altus_chariot_knight"],
    maxRareSpawns: 3,
    boss: "draconic_tree_sentinel",
    length: 15,
    unlocks: ["mount_gelmir", "dominula_village", "bestial_sanctum"],
    hazards: ["folie"],
  },

  caria_mansion: {
    name: "Manoir de Caria",
    monsters: ["lesser_fingercreeper"],
    rareMonsters: ["carian_troll_knight"],
    maxRareSpawns: 3,
    boss: "royal_knight_loretta",
    length: 8,
    unlocks: ["siofra_river"],
  },

  caelid_south: {
    name: "Sud de Caélid",
    monsters: ["radahn_soldier", "giant_dog"],
    rareMonsters: ["giant_crow", "rotten_marionetist"],
    maxRareSpawns: 2,
    boss: "commander_oneil_strong",
    length: 12,
    unlocks: ["redmane_castle"],
  },

  caelid_dragonbarrow: {
    name: "Tertre Draconique",
    monsters: ["giant_dog", "radahn_soldier"],
    rareMonsters: ["rotten_marionetist"],
    // Meme oubli qu'au Chateau du Lion Rouge.
    maxRareSpawns: 2,
    boss: "ekzykes",
    length: 6,
    unlocks: null,
  },

  redmane_castle: {
    name: "Château du Lion Rouge",
    monsters: ["radahn_soldier"],
    rareMonsters: ["winged_paladin"],
    // Sans ce champ, core.js lit `maxRareSpawns || 0` et le Paladin
    // Aile n'apparaissait JAMAIS : ni ses 2500 runes, ni la panoplie du
    // Bourreau, ni le Cri des Astres, seule entree du jeu qui n'avait
    // aucune autre source. Radahn arrivait donc sur un joueur prive du
    // butin de son propre chateau.
    maxRareSpawns: 2,
    boss: "radahn",
    length: 8,
    // unlocks: null,
    unlocks: ["nokron"],
  },

  siofra_river: {
    name: "Rivière Siofra",
    monsters: ["ancestral_follower", "siofra_rat"],
    rareMonsters: ["ancestral_sniper"],
    maxRareSpawns: 2,
    boss: "ancestral_spirit",
    length: 15,
    unlocks: ["redmane_castle"],
  },

  nokron: {
    name: "Nokron, Cité Éternelle",
    monsters: ["silver_tear_nokron", "nox_monk"],
    rareMonsters: ["giant_silver_tear"],
    maxRareSpawns: 2,
    boss: "mimic_tear_boss",
    length: 12,
    unlocks: ["ainsel_river", "deeproot_depths"],
  },
  // ---
  ainsel_river: {
    name: "Rivière Ainsel",
    rareMonsters: ["ainsel_oracle", "malformed_starling"],
    maxRareSpawns: 3,
    monsters: ["ainsel_ant", "ainsel_priest"],
    boss: "dragonkin_ainsel",
    length: 12,
    unlocks: ["rotlake"],
  },

  deeproot_depths: {
    name: "Profondeurs de la Souche",
    rareMonsters: ["siluria_remnant", "deathblight_basilisk"],
    maxRareSpawns: 3,
    monsters: ["root_shambler", "root_guardian"],
    boss: "fia_champion_echo",
    length: 12,
    unlocks: null,
  },
  rotlake: {
    name: "Lac de la Putréfaction",
    rareMonsters: ["cleanrot_revenant", "ulcerated_rot_spirit"],
    maxRareSpawns: 3,
    monsters: ["rot_pest", "scarlet_monk"],
    boss: "astel_bud",
    length: 12,
    unlocks: null,
  },

  mount_gelmir: {
    name: "Mont Gelmir",
    rareMonsters: ["serpent_inquisitor"],
    maxRareSpawns: 2,
    monsters: ["gelmir_hexmage", "altus_omen"],
    boss: "praetor_fragment",
    length: 14,
    unlocks: ["volcano_manor"],
    hazards: ["poison", "putrefaction"],
  },
  leyndell_royal: {
    name: "Leyndell, Cité Royale",
    monsters: ["leyndell_gilded_knight", "leyndell_omen_killer"],
    rareMonsters: ["leyndell_tree_watcher"],
    maxRareSpawns: 2,
    boss: "leyndell_erdtree_avatar",
    length: 14,
    unlocks: ["forbidden_land", "divine_tower", "jarburg"],
    hazards: ["folie"],
  },
  forbidden_land: {
    name: "Terre Interdite",
    monsters: ["forbidden_black_knife", "forbidden_grave_bird"],
    rareMonsters: ["forbidden_deathbird"],
    maxRareSpawns: 2,
    boss: "forbidden_gravekeeper",
    length: 14,
    unlocks: ["mountaintops"],
    hazards: ["gel"],
  },
  mountaintops: {
    name: "Cimes des Geants",
    rareMonsters: ["icy_colossus"],
    maxRareSpawns: 2,
    monsters: ["giant_fire_disciple", "mountaintops_bird"],
    boss: "fire_giant_shard",
    length: 16,
    unlocks: ["crumbling_farum_azula", "consecrated_snowfield"],
    hazards: ["gel", "folie"],
  },
  consecrated_snowfield: {
    name: "Plaine Enneigée Consacrée",
    monsters: ["snowfield_albinauric_rider", "snowfield_frost_hound"],
    rareMonsters: ["snowfield_night_cavalry"],
    maxRareSpawns: 2,
    boss: "snowfield_rime_dragon",
    length: 16,
    unlocks: ["mohgwyn_palace"],
    hazards: ["gel", "folie"],
  },
  mohgwyn_palace: {
    name: "Palais de Mohgwyn",
    monsters: ["mohgwyn_blood_noble", "mohgwyn_albinauric_wretch"],
    rareMonsters: ["mohgwyn_blood_hound"],
    maxRareSpawns: 2,
    boss: "mohgwyn_blood_lord_echo",
    length: 16,
    unlocks: ["miquella_haligtree"],
    hazards: ["poison"],
  },
  miquella_haligtree: {
    name: "Arbre Sacré de Miquella",
    monsters: ["haligtree_misbegotten_crusader", "haligtree_putrid_avatar"],
    rareMonsters: ["haligtree_oracle_envoy"],
    maxRareSpawns: 2,
    boss: "haligtree_rot_knight",
    length: 17,
    unlocks: ["elphael"],
    hazards: ["putrefaction", "folie"],
  },
  /*
   * Epreuves. Volontairement hors du graphe de progression : aucun biome ne
   * les debloque et elles n'en debloquent aucun. On y entre depuis le panneau
   * de fin de partie, jamais par la carte. length: 1 — un seul palier, le boss.
   */
  trial_watcher: {
    name: "Epreuve : Le Veilleur sans Nom",
    monsters: ["trial_watcher_boss"],
    rareMonsters: [],
    maxRareSpawns: 0,
    boss: "trial_watcher_boss",
    length: 1,
    unlocks: [],
    isTrial: true,
  },
  trial_twin: {
    name: "Epreuve : Les Jumeaux d'Ombre",
    monsters: ["trial_twin_boss"],
    rareMonsters: [],
    maxRareSpawns: 0,
    boss: "trial_twin_boss",
    length: 1,
    unlocks: [],
    isTrial: true,
  },
  trial_hollow: {
    name: "Epreuve : La Couronne Creuse",
    monsters: ["trial_hollow_boss"],
    rareMonsters: [],
    maxRareSpawns: 0,
    boss: "trial_hollow_boss",
    length: 1,
    unlocks: [],
    isTrial: true,
  },
  trial_first: {
    name: "Epreuve : Le Premier Sans-Eclat",
    monsters: ["trial_first_boss"],
    rareMonsters: [],
    maxRareSpawns: 0,
    boss: "trial_first_boss",
    length: 1,
    unlocks: [],
    isTrial: true,
  },

  crumbling_farum_azula: {
    name: "Farum Azula en Ruines",
    rareMonsters: ["azula_black_priest"],
    maxRareSpawns: 3,
    monsters: ["azula_beast_lord", "azula_beastman"],
    boss: "azula_tempest_avatar",
    length: 17,
    unlocks: ["farum_azula_deep", "evergaol_champions"],
    hazards: ["folie", "putrefaction"],
  },
  /* ================================================================
     Biomes de la version complete.

     Chacun porte un `traits` : la regle locale qui le distingue des
     autres au-dela de ses points de vie (voir biome-traits.js). Sans
     elle, un biome n'est qu'un palier de chiffres.
     ================================================================ */

  dominula_village: {
    name: "Village de Dominula",
    monsters: ["dominula_celebrant", "dominula_drummer"],
    rareMonsters: ["dominula_maypole_dancer"],
    maxRareSpawns: 2,
    boss: "godskin_apostle",
    length: 13,
    unlocks: ["shaded_castle"],
    hazards: ["folie"],
    traits: ["festival"],
  },
  shaded_castle: {
    name: "Chateau Ombrage",
    monsters: ["shaded_sentry", "shaded_poison_hound"],
    rareMonsters: ["shaded_briar_scion"],
    maxRareSpawns: 2,
    boss: "elemer_briar",
    length: 13,
    unlocks: ["mount_gelmir"],
    hazards: ["poison"],
    traits: ["toxic_mist"],
  },
  volcano_manor: {
    name: "Manoir du Volcan",
    monsters: ["volcano_manservant", "volcano_abductor"],
    rareMonsters: ["volcano_iron_virgin"],
    maxRareSpawns: 2,
    boss: "godskin_noble",
    length: 14,
    unlocks: ["rykard_lair"],
    hazards: ["folie"],
    traits: ["blood_contract"],
  },
  rykard_lair: {
    name: "Antre de Rykard",
    monsters: ["rykard_serpent_spawn", "rykard_devoured"],
    rareMonsters: ["rykard_blasphemous_priest"],
    maxRareSpawns: 2,
    boss: "rykard_lord_blasphemy",
    length: 14,
    unlocks: ["leyndell_royal"],
    hazards: ["folie"],
    traits: ["serpent_blood"],
  },
  divine_tower: {
    name: "Tour Divine d'Altus",
    monsters: ["divine_tower_watch", "divine_tower_oracle"],
    rareMonsters: ["divine_tower_gargoyle"],
    maxRareSpawns: 2,
    boss: "divine_tower_keeper",
    length: 15,
    unlocks: ["castle_sol"],
    hazards: ["folie"],
    traits: ["sealed_grace"],
  },
  castle_sol: {
    name: "Chateau Sol",
    monsters: ["sol_banished_knight", "sol_night_stalker"],
    rareMonsters: ["sol_eochaid_wraith"],
    maxRareSpawns: 2,
    boss: "commander_niall",
    length: 15,
    unlocks: ["giants_catacombs"],
    hazards: ["gel"],
    traits: ["eternal_night"],
  },
  giants_catacombs: {
    name: "Catacombes des Geants",
    monsters: ["catacomb_ember_shade", "catacomb_grave_giant"],
    rareMonsters: ["catacomb_ancient_hero"],
    maxRareSpawns: 2,
    boss: "catacomb_burnt_spirit",
    length: 15,
    unlocks: ["mountaintops"],
    hazards: ["gel", "folie"],
    traits: ["ember_field"],
  },
  elphael: {
    name: "Elphael, Rempart de l'Arbre",
    monsters: ["elphael_cleanrot_knight", "elphael_rot_swarm"],
    rareMonsters: ["elphael_rot_dog"],
    maxRareSpawns: 2,
    boss: "malenia_blade",
    length: 17,
    unlocks: ["crumbling_farum_azula"],
    hazards: ["putrefaction"],
    traits: ["scarlet_bloom"],
  },
  farum_azula_deep: {
    name: "Farum Azula, Abime",
    monsters: ["azula_dragon_warrior", "azula_storm_hawk"],
    rareMonsters: ["azula_maliketh"],
    maxRareSpawns: 2,
    boss: "placidusax",
    length: 17,
    unlocks: ["leyndell_ash"],
    hazards: ["folie"],
    traits: ["tempest"],
  },
  leyndell_ash: {
    name: "Leyndell la Cendreuse",
    monsters: ["ash_gilded_guard", "ash_putrid_avatar"],
    rareMonsters: ["ash_gideon_ozz"],
    maxRareSpawns: 2,
    boss: "hoarah_loux",
    length: 18,
    unlocks: ["erdtree_throne"],
    hazards: ["putrefaction", "folie"],
    traits: ["ashen_veil"],
  },
  erdtree_throne: {
    name: "Trone d'Elden",
    monsters: ["throne_golden_shade", "throne_order_fragment"],
    rareMonsters: ["throne_radagon"],
    maxRareSpawns: 2,
    boss: "elden_beast",
    length: 20,
    unlocks: [],
    hazards: ["folie"],
    traits: ["no_retreat"],
  },

  /* --- Zones annexes : butin et creatures emblematiques ---------- */

  bestial_sanctum: {
    name: "Sanctuaire Bestial",
    monsters: ["bestial_cleric", "bestial_vulture"],
    rareMonsters: ["bestial_black_blade"],
    maxRareSpawns: 0,
    boss: "gurranq_beast_clergyman",
    length: 12,
    unlocks: [],
    hazards: [],
    traits: ["beast_offering"],
  },
  jarburg: {
    name: "Jarburg",
    monsters: ["jarburg_living_jar", "jarburg_jar_bairn"],
    rareMonsters: ["jarburg_alexander_shard"],
    maxRareSpawns: 3,
    boss: "jarburg_great_jar",
    length: 10,
    unlocks: [],
    hazards: [],
    traits: ["jar_luck"],
  },
  evergaol_champions: {
    name: "Enclos des Champions",
    monsters: ["evergaol_fortissax", "evergaol_astel"],
    rareMonsters: ["evergaol_ancient_dragon"],
    maxRareSpawns: 2,
    boss: "evergaol_nameless_champion",
    length: 6,
    unlocks: [],
    hazards: ["folie"],
    traits: ["gauntlet"],
  },
};

