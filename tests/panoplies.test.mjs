...
test("la normalisation rend toujours exactement trois emplacements", () => {
  for (const entree of [undefined, null, [], "x", [{}], new Array(9).fill({})]) {
...
test("un profil ancien recoit des emplacements vides", () => {
  const out = lo.normaliserPanoplies(profil.normalizePlayerProfile({}).loadouts);
...
test("un identifiant forge est refuse comme partout ailleurs", () => {
  const out = lo.normaliserPanoplies([
    { nom: "Triche", weapon: "<img onerror=1>", armor: "../../x", accessory: "plate_armor" },
  ]);
...
test("un nom est borne en longueur et jamais vide", () => {
  const out = lo.normaliserPanoplies([{ nom: "x".repeat(200), weapon: "fists" }]);
  assert.ok(out[0].nom.length <= lo.NOM_PANOPLIE_MAX);

  const sansNom = lo.normaliserPanoplies([{ nom: "   ", weapon: "fists" }]);
  assert.ok(sansNom[0].nom.trim().length > 0, "un nom vide doit retomber sur un defaut");
});

test("capturer puis resoudre restitue l'equipement a l'identique", () => {
  const etat = {
    equipped: { weapon: "sword_broken", armor: "plate_armor", accessory: "bog_amulet" },
    equippedAsh: "bloody_slash",
  };
  const panoplie = lo.capturerPanoplie(etat, "Tank");
  const { applicable, manquants } = lo.resoudrePanoplie(panoplie, () => true, () => true);
...
test("une piece non possedee est signalee, pas equipee", () => {
  ...
  const { applicable, manquants } = lo.resoudrePanoplie(panoplie, possede, () => false);

  assert.equal(applicable.weapon, "sword_broken");
  assert.equal(applicable.armor, null, "une piece perdue ne doit pas etre equipee");
...
test("l'etat actif se detecte sur les quatre emplacements", () => {
  ...
  const exacte = { nom: "x", weapon: "a", armor: "b", accessory: "c", ash: "d", vide: false };
