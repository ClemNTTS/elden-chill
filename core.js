...
const handleVictory = (sessionId) => {
  ...
  ActionLog(
    `Vous avez vaincu ${firstEnemy.name} ! (+${formatNumber(totalRunes)} runes)`,
    "log-runes",
  );
  ...
  setTimeout(() => {
    ...
    if (!playerStatus.skipTurn) {
      ...
      for (let i = 0; i < stats.attacksPerTurn; i++) {
        ...
        runtimeState.currentEnemyGroup[0].currentHp -= Math.floor(damage);
        updateHealthBars();
    ...
    if (runtimeState.currentEnemyGroup[0].currentHp <= 0) {
      const defeatedEnemy = runtimeState.currentEnemyGroup.shift(); // Remove the defeated enemy
      ...
      if (runtimeState.currentEnemyGroup.length === 0) {
        ...
        // Update the enemy name display to show updated group size
...
const spawnMonster = (monsterId, sessionId) => {
  ...
  for (let i = 0; i < groupSize; i++) {
    const enemy = {
      ...template,
      currentHp: Math.floor(template.hp * multiplier),
